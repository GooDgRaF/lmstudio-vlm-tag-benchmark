...
`configs/config.smoke.yaml` — основной быстрый runtime-профиль: одна модель, одна картинка, все шесть режимов.
...
- `config.yaml` — user-facing simple config, generated by `python main.py init-config`.
- `configs/config.smoke.yaml` — quick verification and development checks.
- `configs/config.example.yaml` — full benchmark config with all active model variants.
...
## Encoding hygiene (Windows)

- Keep text files in UTF-8. For `.md` on Windows editors, UTF-8 with BOM is acceptable.
- In PowerShell, always set encoding explicitly when reading/writing text:
