...
def test_build_rest_input_items_shape():
    out = build_rest_input_items("prompt", "data:image/jpeg;base64,abc")
    assert out == [
...
def test_chat_rest_posts_expected_body(tmp_path, monkeypatch):
    ...
    client.chat_rest(
        model_id="inst1",
        input_items=build_rest_input_items("p", "data:image/jpeg;base64,x"),
        temperature=0.0,
        top_p=1.0,
    ...
    assert seen["json"]["input"][0] == {"type": "text", "content": "p"}
